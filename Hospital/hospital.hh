...
private:
    // a map to contain patient id:s and person classes for current patients
    std::map<std::string, Person*> current_patients_;
    // A map to contain staff id:s and person classes for recruited staff
    std::map<std::string, Person*> staff_;

    // The 'mordor' data structure. The root of my problems and my hated enemy
    // I made this data structure with some grand idea of information
    // storage on all patients. If i could, I would have deleted it ages ago.
    // Unfortunately the map is essential for most of my methods
    // This data structure is extremely annoying concering memory allocation
    // My current solution works but i would replace this structure given enough time.

    // Essentially stores a vector containing person and careperiod classes under
    // patient_ids. Easy to access all relevant information on patients from one structure
    std::map<std::string, std::vector<std::pair<Person*, CarePeriod*>>> patients_;

    // A data structure to store all care periods
    std::vector<CarePeriod*> care_periods_;
    // a set of unique patiets. Exists for memory deallocation.
    // The primary reason for this set is patients_ flaws.
    std::set<Person*> patient_list;
    // all patient ids that have assigned staff in chronological order.
